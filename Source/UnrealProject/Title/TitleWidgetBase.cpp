...
void UTitleWidgetBase::ConnectServer()
{
	...
	{
		SetInfoText(TEXT("현재 접속 가능한 게임 서버가 없습니다. 다시 로그인해 주세요."));
		return;
	}
...
void UTitleWidgetBase::SaveData()
{
	...
	if (GI)
	{
		...
		if (MySubSystem)
		{
			...
			MySubSystem->WebServerHost = MySubSystem->ServerIP;
		}
	}
}
...
void UTitleWidgetBase::ProcessLoginResult(const bool bInSuccess, const FString& InMessage)
{
	...
	if (Data)
	{
		...
		{
			SetInfoText(InMessage.IsEmpty()
				? TEXT("로그인했습니다. 현재 접속 가능한 게임 서버가 없습니다.")
				: InMessage);
		}
		else
		{
			SetInfoText(FString::Printf(TEXT("%s (Lv.%d) - 게임 서버 %s"),
